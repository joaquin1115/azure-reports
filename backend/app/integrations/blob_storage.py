...
async def subir_pdf(pdf_bytes: bytes, nombre_blob: str) -> str:
    ...
    async with BlobServiceClient.from_connection_string(
        ...
        await blob.upload_blob(pdf_bytes, overwrite=True, content_settings={"content_type": "application/pdf"})
    return nombre_blob


async def generar_sas_url(nombre_blob: str, expiry_hours: int = 24) -> str:
    """Generates a time-limited SAS URL for downloading a PDF."""
    from azure.storage.blob import BlobServiceClient as SyncClient
    sync_client = SyncClient.from_connection_string(settings.azure_storage_connection_string)
    ...
    return f"https://{account_name}.blob.core.windows.net/{settings.azure_storage_container}/{nombre_blob}?{sas_token}"
